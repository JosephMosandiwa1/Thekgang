...
const kpis = [
  ...
  { label: 'Stakeholders', value: '0', sublabel: 'Registry building', color: 'border-accent/30 bg-accent/5', valueColor: 'text-accent' },
];
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-muted mt-1">Thekgang NPC — Book Publishing, Manufacturing &amp; Distribution Cluster</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {kpis.map(card => (
          <div key={card.label} className={`border p-5 rounded ${card.color}`}>
            <p className="text-[10px] uppercase tracking-[0.15em] text-muted">{card.label}</p>
            <p className={`text-2xl font-bold mt-2 ${card.valueColor}`}>{card.value}</p>
            <p className="text-[10px] text-muted/60 mt-1">{card.sublabel}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Upcoming Deadlines */}
        <div className="lg:col-span-1 border border-sand/60 rounded p-6">
          <h2 className="text-xs font-semibold uppercase tracking-[0.15em] text-muted mb-4">Upcoming Deadlines</h2>
          <div className="space-y-0">
            {upcomingDeadlines.map(d => (
              <div key={d.item} className="flex items-start gap-3 py-3 border-b border-sand/30 last:border-0">
                ...
                <div>
                  <p className="text-sm text-ink">{d.item}</p>
                  <p className="text-[10px] text-muted/50">{d.date} &middot; {d.type}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Active Programmes */}
        <div className="lg:col-span-2 border border-sand/60 rounded p-6">
          <h2 className="text-xs font-semibold uppercase tracking-[0.15em] text-muted mb-4">Programmes</h2>
          <div className="space-y-0">
            {programmes.map(p => (
              <div key={p.name} className="flex items-center justify-between py-3 border-b border-sand/30 last:border-0">
                <div>
                  <p className="text-sm text-ink font-medium">{p.name}</p>
                  <p className="text-[10px] text-muted/50">{p.location}</p>
                </div>
      ...
      {/* Board */}
      <div className="mt-8 border border-sand/60 rounded p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={m.name} className="flex items-center gap-3 p-3 bg-warm-gray/30 rounded">
              <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center text-xs font-semibold text-accent">{m.initials}</div>
              <div>
