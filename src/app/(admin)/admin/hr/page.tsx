  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-display font-bold text-ink">HR &amp; Team</h1>
          <p className="text-sm text-muted mt-1">Staff register, contracts, timesheets, leave</p>
        </div>
        <button onClick={() => setShowAdd(true)} className="bg-accent text-white text-[10px] font-medium tracking-wider px-5 py-2.5 uppercase rounded hover:bg-accent-light transition-colors">+ Add Staff</button>
      </div>

      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: 'Total Staff', value: staff.length, color: 'text-ink' },
          ...
          <div key={c.label} className="border border-sand/60 rounded p-4">
      ...
      <div className="border border-sand/60 rounded">
        ...
        {staff.length === 0 ? (
          <div className="px-6 py-12 text-center text-muted/50 text-sm">{loading ? 'Loading...' : 'No staff registered yet'}</div>
        ) : staff.map(s => (
          <div key={s.id} className="grid grid-cols-12 gap-2 px-6 py-3 border-t border-sand/30 items-center text-sm hover:bg-warm-gray/20 transition-colors">
            ...
            <span className="col-span-3 text-ink/60 text-xs">{s.role}</span>
            <span className="col-span-2 text-muted text-xs">{s.email || '—'}</span>
            <span className="col-span-2 text-muted text-xs capitalize">{s.contract_type}</span>
            <span className="col-span-2 text-muted text-xs">{s.start_date || '—'}</span>
          </div>
        ))}
      </div>
        ...
        <>
          ...
          <div className="fixed inset-0 z-[101] flex items-center justify-center p-6">
            <div className="bg-white border border-sand/60 rounded-lg shadow-xl w-full max-w-md p-6">
              <h3 className="text-base font-display font-bold text-ink mb-4">Add Staff Member</h3>
              <div className="space-y-3">
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Full name" className="w-full border border-sand/60 px-3 py-2 text-sm rounded focus:outline-none focus:border-accent" />
                ...
                <input value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))} placeholder="Role / Title" className="w-full border border-sand/60 px-3 py-2 text-sm rounded focus:outline-none focus:border-accent" />
                <select value={form.contractType} onChange={e => setForm(f => ({ ...f, contractType: e.target.value }))} className="w-full border border-sand/60 px-3 py-2 text-sm rounded focus:outline-none focus:border-accent">
