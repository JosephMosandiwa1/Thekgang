...
export default function FinancePage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-display font-bold text-black">Finance</h1>
        <p className="text-sm text-gray-500 mt-1">DSAC budget tracking, expenses, invoices, ledger</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'DSAC Allocation', value: 'R 2 000 000', color: 'text-black', bg: 'border-gray-200/60' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="border border-gray-200/60 rounded p-6">
          <h2 className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-500 mb-4">Quick Actions</h2>
          <div className="space-y-2">
            {['Record Expense', 'Create Invoice', 'View Ledger', 'Budget vs Actual', 'DSAC Financial Report'].map(a => (
              <button key={a} className="w-full text-left px-4 py-3 text-sm border border-gray-200/40 rounded hover:bg-gray-100/30 hover:border-black/20 transition-colors">{a}</button>
            ))}
          </div>
        </div>

        <div className="border border-gray-200/60 rounded p-6">
          <h2 className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-500 mb-4">Budget Allocation (DSAC MOA)</h2>
            ...
            {[
              ...
              <div key={b.item}>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-black/60">{b.item}</span>
                  <span className="text-black font-medium">{fmt(b.amount)} ({b.percent}%)</span>
                </div>
                <div className="h-2 bg-sand/40 rounded-full overflow-hidden">
                  <div className="h-full bg-black rounded-full" style={{ width: `${b.percent}%` }} />
                </div>
              </div>
            ))}
          </div>
