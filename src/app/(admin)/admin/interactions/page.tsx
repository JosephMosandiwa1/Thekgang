    ...
    <div>
      ...
      <div className="border border-gray-200/60 rounded">
        {interactions.length === 0 ? (
          <div className="px-6 py-12 text-center text-gray-500/50 text-sm">{loading ? 'Loading...' : 'No interactions logged yet — add them from stakeholder profiles'}</div>
        ) : interactions.map(i => (
          <div key={i.id} className="flex gap-4 px-6 py-4 border-b border-gray-200/30 last:border-0 hover:bg-gray-100/20 transition-colors">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-gray-500/50">{i.date?.split('T')[0]}</span>
              </div>
              ...
              {i.follow_up && (
                <div className="mt-2 flex items-center gap-2">
                  ...
                  {i.follow_up_date && <span className="text-[10px] text-gray-500/50">by {i.follow_up_date}</span>}
                </div>
              )}
            </div>
          </div>
