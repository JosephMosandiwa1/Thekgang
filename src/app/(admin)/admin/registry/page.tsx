...
export default function RegistryPage() {
  ...
  return (
    <div>
      ...
      <input type="text" placeholder="Search by name or email..." value={search} onChange={e => setSearch(e.target.value)} className="bg-white border border-gray-200/60 px-4 py-2 text-sm rounded text-black placeholder:text-gray-500/50 focus:outline-none focus:border-black w-full max-w-sm mb-6" />
      ...
      <div className="border border-gray-200/60 rounded">
        ...
        {filtered.length === 0 ? (
          <div className="px-6 py-12 text-center text-gray-500/70 text-sm">{loading ? 'Loading...' : 'No registrations yet'}</div>
        ) : filtered.map(s => (
